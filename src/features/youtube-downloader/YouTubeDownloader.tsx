...
import { Youtube, Download, Link as LinkIcon, FileVideo, Music, Info } from 'lucide-react';
...
export default function YouTubeDownloader() {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const response: any = await invoke('get_youtube_info', { url });
      ...
      if (response.success) {
        ...
        alert(`Failed to fetch video info: ${response.error}`);
      }
    } catch (err: any) {
      console.error('Fetch error:', err);
      alert(`Critical error in renderer: ${err.message || err}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      const response: any = await invoke('download_youtube_video', { url, format });
      if (response.success) {
          ...
          <aside className="lg:col-span-4">
            <div className="glass-card rounded-[32px] p-6 space-y-6">
              <div className="flex flex-col gap-4">
                ...
                >
                  <FileVideo size={20} />
                  <span className="font-black text-[10px] tracking-widest uppercase text-left">
                    Video (MP4)
                  </span>
                </button>
                <button
