        ...
        <header className="space-y-6 pt-10 md:pt-20">
          <div className="space-y-4">
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter text-white leading-[0.9] flex flex-col">
              <span>Zen<span className="text-indigo-500 italic">Convert</span></span>
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        color === 'indigo' ? "text-indigo-400" : color === 'rose' ? "text-rose-400" : "text-amber-400"
      )}>
        {icon}
      </div>
      <div className="space-y-3">
        <div className="flex items-center gap-2">
            ...
            <ChevronRight size={20} className="text-slate-600 group-hover:translate-x-1 group-hover:text-white transition-all" />
        </div>
        <p className="text-slate-500 font-medium leading-relaxed">{description}</p>
      </div>
      
      {/* Visual Accent */}
      <div className={`absolute -bottom-10 -right-10 w-32 h-32 bg-${color}-500/10 blur-[60px] rounded-full group-hover:bg-${color}-500/20 transition-all`} />
    </div>
  );
}
